...
export async function GET(request: NextRequest) {
  try {
    ...
    const foundIdCards = await getIdCardIssuesByUidAndDate(
      uid,
      date ?? undefined
    );
    console.log("foundIdCards:", foundIdCards);
    if (!foundIdCards || foundIdCards.length === 0) {
      // Added check for empty array
      return new NextResponse("No id card found for the given uid", {
        status: 404,
      });
    }
